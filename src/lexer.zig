...
test "test multiple in command" {
    ...
    const expectedTokens = [_]Token{
        .{ .Cmd = Command{ .name = "cmake_minimum_required" } },
        .{ .Paren = Paren{ .opener = true } },
        .{ .UnquotedArg = UnquotedArg{ .name = "VERSION" } },
        .{ .UnquotedArg = UnquotedArg{ .name = "3.16" } },
        .{ .UnquotedArg = UnquotedArg{ .name = "FATAL_ERROR" } },
        .{ .Paren = Paren{ .opener = false } },
    };
...
test "test multiple parens in command" {
    const t = std.testing;
    var tokens = try lex("if((CMAKE_CXX_COMPILER_ID STREQUAL \"GNU\") AND (${CMAKE_SYSTEM_NAME} MATCHES \"Linux\"))", t.allocator);
    defer tokens.clearAndFree();
    ...
    const expectedTokens = [_]Token{
        ...
        .{ .Paren = Paren{ .opener = true } },
        .{ .Paren = Paren{ .opener = true } },
        .{ .UnquotedArg = UnquotedArg{ .name = "CMAKE_CXX_COMPILER_ID" } },
        .{ .UnquotedArg = UnquotedArg{ .name = "STREQUAL" } },
        .{ .QuotedArg = QuotedArg{ .name = "\"GNU\"" } },
        .{ .Paren = Paren{ .opener = false } },
        .{ .UnquotedArg = UnquotedArg{ .name = "AND" } },
        .{ .Paren = Paren{ .opener = true } },
        .{ .UnquotedArg = UnquotedArg{ .name = "${CMAKE_SYSTEM_NAME}" } },
        .{ .UnquotedArg = UnquotedArg{ .name = "MATCHES" } },
        .{ .QuotedArg = QuotedArg{ .name = "\"Linux\"" } },
        .{ .Paren = Paren{ .opener = false } },
        .{ .Paren = Paren{ .opener = false } },
    };
    try t.expectEqualDeep(expectedTokens[0..], tokens.items[0..]);
...
test "test escaped unquoted arg" {
    const t = std.testing;
    var tokens = try lex("set(VAR \\\"\\\")", t.allocator);
    defer tokens.clearAndFree();
    //     std.debug.print("{any}\n", .{tokens});
    const expectedTokens = [_]Token{
        .{ .Cmd = Command{ .name = "set" } },
        .{ .Paren = Paren{ .opener = true } },
        .{ .UnquotedArg = UnquotedArg{ .name = "VAR" } },
        .{ .UnquotedArg = UnquotedArg{ .name = "\\\"\\\"" } },
        .{ .Paren = Paren{ .opener = false } },
    };
    try t.expectEqualDeep(expectedTokens[0..], tokens.items[0..]);
}
...
test "comment in args" {
    ...
    const t = std.testing;
    var tokens = try lex(source, std.testing.allocator);
    defer tokens.clearAndFree();
    const expectedTokens = [_]Token{
        .{ .Cmd = .{ .name = "if" } },
        .{ .Paren = .{ .opener = true } },
